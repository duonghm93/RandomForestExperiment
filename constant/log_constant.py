...
FEATURE_ORDER_CHANGE_EXPERIMENT_FOLDER = 'feature_order_change_exp/'
import time
...
def get_feature_order_change_experiment_log_folder():
    return LOG_FOLDER + FEATURE_ORDER_CHANGE_EXPERIMENT_FOLDER
...
def get_feature_order_change_experiment_log_file(spid=6968, addition_infor=''):
    current_time = time.strftime('%Y_%m_%d_%H_%M_%S')
    return get_feature_order_change_experiment_log_folder() + str(
        spid) + '_' + addition_infor + '_' + current_time + '.csv'